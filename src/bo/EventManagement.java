...
public class EventManagement {
    ...
    public boolean update(int ID, String newNameEvent, Date newDate,
            String newLocation, int  newNumberOfAttend, Status  newStatus) {
        if (!isExistEvent(ID)) {
            return false;
        }
        Event oldEvent = getEventByID(ID);
        if (!newNameEvent.trim().isEmpty()) {
            oldEvent.setNameEvent(newNameEvent);
        }
        if (newDate!=null) {
